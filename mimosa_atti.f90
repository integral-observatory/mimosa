...
SUBROUTINE ROTAT_LOOP_ONE(mf,is,rot_Type,tc,&
            ...
            carte,cartevar ,exposure,surf,&
            arefimage,drefimage,lrefimage,brefimage,aref,dref,lref,bref,&
...
Real(KIND=8) ::arefimage,drefimage,lrefimage,brefimage,aref,dref,lref,bref
...
! ISDC INTERFACE
!-----------------
!!$call message(procname,'ISDC INTERFACE',0,status)
...
!!$   Endif
  ...
  data_type=2
   ...
   Do poi=1,point_num 
      if((ActiveScwArray(poi)==1).or.(poi==point_num))then
...
!!$      if (poi .gt.point_num-5)then
!!$         write(str250,*)' poi, alphaXtel_deg,deltaXtel_deg :',poi, alphaXtel_deg,deltaXtel_deg
...
case(1,4,6,78,9,10,11,12,13,14,15,16,17,19)
!FLUX SPREAD - SURFACE
  id = 0
  ce = 0.0
  cb = 0.0
...
!!$      end select
      ...
      call IMAGE_TO_SKY_PROJECTION(InputProjType(num1),tc,pixel1,pixel2,&
                     pixel_ang1,pixel_ang2,&
                     arefimage,drefimage,lrefimage,brefimage,&
                      aref,dref,lref,bref,&
                      rot_mat1,rot_mat2,rot_mat3,rot_mat4,&
   ...
   ! in,jn - input mosaics centre projection
   i = idim1/2
   j = jdim1/2
   ri = Real(i)-cb-i_cen_inp - dispi_in 
   rj=Real(j)-cb-j_cen_inp   - dispj_in 
   ...
   if((mosaangsize > 180).and.((in < imin).or.(in > imax )) )then
...
do j=jmin,jmax
   ...
   do i=imin,imax
      ...
      call SKY_TO_IMAGE_PROJECTION(InputProjType(num1),tc,pixel1,pixel2,&
                     pixel_ang1,pixel_ang2,&
                      arefimage,drefimage,lrefimage,brefimage,&
                      aref,dref,lref,bref,&
                      rot_mat1,rot_mat2,rot_mat3,rot_mat4,&
          ...
          If(MIN0(in,jn,i_dim_out-in+id,&
            ...
            case(2)
               ...
               ! PIXEL UPPER RIGHT CORNER
                 ...
                 call SKY_TO_IMAGE_PROJECTION&
                      (InputProjType(num1),tc,pixel1,pixel2,&
                     pixel_ang1,pixel_ang2,&
                     arefimage,drefimage,lrefimage,brefimage,&
                      aref,dref,lref,bref,&
                      rot_mat1,rot_mat2,rot_mat3,rot_mat4,&
...
!!$ if((i==359).and.(j==398))then
!!$         print '(I3,1x,2(I4,1x),1x,2(I4,1x),2x,2(F5.2,1x,g18.8,1x,f6.3,2x))',num1,i,j,in,jn,&
!!$              image(in,jn),imagevar(in,jn), image(in,jn)/sqrt(imagevar(in,jn)),&
!!$              carte(i,j),cartevar(i,j), carte(i,j)/sqrt(cartevar(i,j ))  
!!$      write(13,'(I3,1x,2(I4,1x),1x,2(I4,1x),2x,2(F5.2,1x,g18.8,1x,f6.3,2x))')  num1,i,j,in,jn,&
!!$              image(in,jn),imagevar(in,jn), image(in,jn)/sqrt(imagevar(in,jn)),&
!!$              carte(i,j),cartevar(i,j), carte(i,j)/sqrt(cartevar(i,j ))   
!!$      endif
